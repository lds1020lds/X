...
void CreateTestCase( )
{
	...
	nfa_case* ct18_1 = new nfa_case("^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,16}$", false);
	ct18_1->AddTestCase("lds32LDS", true);
	ct18_1->AddTestCase("33331333", false);
	ct18_1->AddTestCase("lllldsdsfs", false);
	ct18_1->AddTestCase("LLLLLLLLL", false);
	ct18_1->AddTestCase("ldsdff333", false);
	ct18_1->AddTestCase("FDFGDFldsa", false);
	ct18_1->AddTestCase("A157544455d", true);
	ct18_1->AddTestCase("1256LDfdgd85", true);
	ct18_1->AddTestCase("UYTldssd998", true);

	cases.push_back(ct18_1);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	CreateTestCase();
	// 设置工作目录为 XScriptTest 目录
	SetCurrentDirectoryA("C:/Projects/XScriptSVN/trunk/XScript/XScriptTest");

	bool syntaxOnly = false;
	bool recoverMode = false;
	bool waitDebugger = false;
	int debugPort = 5678;
	//const char* fileName = "testcases/iterator/test_iterator.xs";//"comprehensive_tests/test_reg.xs";//"test_trycatch_rethrow_limit.xs";
	const char* fileName = "comprehensive_tests/test_list_operators.xs";  

	//const char* fileName = "testcases/test_all.xs";
	
	for (int i = 1; i < argc; i++)
	{
		if (_tcscmp(argv[i], _T("--syntax-only")) == 0 || _tcscmp(argv[i], _T("--parse-only")) == 0)
		{
			syntaxOnly = true;
		}
		else if (_tcscmp(argv[i], _T("--recover")) == 0)
		{
			recoverMode = true;
		}
		else if (_tcscmp(argv[i], _T("--wait-debugger")) == 0)
		{
			waitDebugger = true;
		}
		else if (_tcscmp(argv[i], _T("--debug-port")) == 0 && i + 1 < argc)
		{
			debugPort = _ttoi(argv[++i]);
		}
		else
		{
#ifdef UNICODE
			static char fileNameBuffer[MAX_PATH];
			WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, fileNameBuffer, MAX_PATH, NULL, NULL);
	...
	gScriptVM.init();
	XScriptDebugger debugger;
	if (!syntaxOnly && debugPort > 0)
	{
		gScriptVM.SetDebugger(&debugger);
		debugger.StartServer(debugPort, waitDebugger);
	}
	if (syntaxOnly)
	{
		CSourceFile sourceFile;
		if (!sourceFile.LoadSourceFile(fileName))
		{
		...
		bool succ = parser.ParseFile(&sourceFile, &midCode, &symbolTable, recoverMode, true);
		if (succ)
		{
			printf("Syntax OK: %s\n", fileName);
			return 0;
		...
		const std::vector<CParser::ParseError>& errors = parser.GetParseErrors();
		for (int i = 0; i < (int)errors.size(); i++)
		{
			printf("Syntax Error:%s at line(%d),char(%d)\n", errors[i].message.c_str(), errors[i].line, errors[i].character);
		}
		return 1;
	}

	gScriptVM.doFile(fileName);
